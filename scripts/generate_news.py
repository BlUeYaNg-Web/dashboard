...
def select_articles(client, today, category, articles, needs_pitch):
    ...
    # 銷售觀念背景（用於生成 sales_pitch）
    project_context = """【銷售觀念】
- 地點：新北市土城區，稀缺地段，未來 5 年周邊不會再有比這更好地段的新建案
- 交通：LG 捷運線未來大站，帶動區域長期增值
- 產品：20～33坪預售屋，總價相對親民
- 客群：有資產的首購族、置產客
- 核心訴求：好的產品，價值超越價格——現在買的是未來還沒被市場定價的稀缺性
- 注意：不要提建案名稱、不要提「青埔特區」（那是桃園）"""

    pitch_field = (
        f'\n        "sales_pitch": "口語化銷售觀念（1-2句，從新聞引導出『好產品價值超越價格』的觀念，像朋友聊天而非廣告）",'
        if needs_pitch else ""
    )
    pitch_rule = """6. sales_pitch 要求：
   - 核心銷售邏輯有兩層，擇一或融合使用：
     【一】價值超越價格：現在的買家看的不是便不便宜，而是這個產品值不值得擁有
     【二】我們不是在找買家，我們是在幫已經買的客人挑鄰居——這裡的門檻不是錢，是眼光
   - 語氣要像一個有自信的朋友，在給對方一個可以考慮的觀念，不是在推銷
   - 從新聞切入，自然帶出這個觀念，不要硬扯
   - 不超過 2 句，口語，不要書面廣告腔，不要提建案名稱""" if needs_pitch else ""
